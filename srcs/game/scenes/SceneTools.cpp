...
void SceneTools::_displayTimer(GUI *graphicUI, float *currentTime, bool isPause) {
	...
	if (graphicUI->uiStartBlock(
		...
		std::string minutes = tmpMinutes == 0 ? "00" : std::to_string(tmpMinutes);
		int tmpSec = static_cast<int>(*currentTime) % 60;
		std::string sec = tmpSec == 0 ? "00" : std::to_string(tmpSec);
		graphicUI->uiHeader((minutes + " : " + sec).c_str(), NK_TEXT_CENTERED, 60,
							"30_BOMBERMAN");
	}
	graphicUI->uiEndBlock();
}
