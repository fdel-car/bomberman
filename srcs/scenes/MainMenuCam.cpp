...
void MainMenuCam::drawGUI(GUI * graphicUI) {
	...
	if (graphicUI->uiStartBlock(
			"MainMenu2", "",
			nk_rect(xPos - (xPos / 2), (WINDOW_H / 5) * 4, (WINDOW_W / 5), 60),
			NK_WINDOW_NO_SCROLLBAR)) {
		if (graphicUI->uiHover()) {
			if (graphicUI->uiButton((WINDOW_W / 5), 60, 0, "Settings", "settings", "14_BOMBERMA"))
				std::cout << "3" << std::endl;
		} else {
			if (graphicUI->uiButton((WINDOW_W / 5), 60, NK_TEXT_RIGHT, "Settings", "",
									"14_BOMBERMA")) {
				std::cout << "3" << std::endl;
				_newSceneIdx = 1;
			}
	...
	if (graphicUI->uiStartBlock(
			...
			NK_WINDOW_NO_SCROLLBAR)) {
		if (graphicUI->uiButton((WINDOW_W / 5), 60, NK_TEXT_RIGHT, "Level 1", "", "14_BOMBERMA")) {
			std::cout << "2" << std::endl;
			_newSceneIdx = 1;
		}
	}
	...
	if (graphicUI->uiStartBlock("MainMenu4", "",
								nk_rect(xPos * 4 - (xPos / 2),
